...
const LotteryListLoader = () => {
  ...
  return (
    <div className="my-6">
      ...
      {<LotteryListWithLoading isLoading={loading} lotteries={lotteries} />}
    </div>
  );
};
...
const LotteryList = ({lotteries}) => {
  return lotteries.length > 0 ? (
    ...
    <Message
      message="There is no available lotteries at the moment, come back later!"
      color="red-600"
    />
  );
};

const Lottery = ({id, ticket, jackpot, endTime, state}) => {
  ...
  return (
    <div className="h-85 my-4 border-2 border-light-green shadow-lg shadow-light-green rounded-2xl bg-back-black my-6 p-4 flex flex-col justify-between">
      <div className="mb-4">
        <div className="flex flex-col text-center text-3xl text-light-green">
          <h3>Prize</h3>
          <div className="flex justify-center  mb-4 items-center">
            <img src={logo} alt="" className="h-auto h-12" />
            <span className="text-white text-6xl pl-4 digital-font">
              {formatToEther(jackpot)}
            </span>
          </div>
        </div>
        <div className="text-center text-custom-gray font-bold text-2xl italic">
          {lotteryState}
        </div>
        <ul>
          <li className="uppercase text-custom-gray">
            ends at: <span className="text-white">{getDate(endTime)}</span>
          </li>
          <li className="uppercase text-custom-gray">
            participants:{" "}
          ...
          <li className="uppercase text-custom-gray">
            Ticket price:
            <span className="text-white">{formatToEther(ticket)} MATIC</span>
          </li>
        </ul>
      </div>
      <ParticipateButton
...
const ParticipateButton = ({
  ...
  if (lotteryState !== "OPEN") {
    ...
    account &&
    participants.includes(ethers.utils.getAddress(account))
  ) {
    return (
      <div className="text-2xl p-3 mx-auto">You've participated !</div>
    );
  } else {
    return (
      <button
