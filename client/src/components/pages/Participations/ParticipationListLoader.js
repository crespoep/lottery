  ...
  return (
    <>
      {participations.length > 0 ? (
        ...
        <Message
          message="You have no participations in any lottery game"
          color="hard-yellow"
        />
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 my-6 text-white">
        {filteredParticipations.map((lottery) => (
          <Participation key={lottery.id.toNumber()} {...lottery} />
        ))}
      </div>
    </>
  );
...
const Participation = ({jackpot, id, ticket, winner}) => {
  const formatToEther = (number) => ethers.utils.formatEther(number);

  const isThereAWinner = () => winner !== ethers.constants.AddressZero;

  return (
    <div className="h-60 my-4 border-2 border-light-green shadow-lg shadow-light-green rounded-2xl bg-back-black my-6 p-4 flex flex-col justify-between">
